...
class _PackageScreenState extends State<PackageScreen> {
      ...
      body: Stack(
        children: [
          Container(
            ...
            // === THAY ĐỔI 2: CO GIÃN GIAO DIỆN CHUNG ===
            child: Center(
              child: ConstrainedBox(
                constraints: const BoxConstraints(maxWidth: 960), // Giới hạn chiều rộng tổng thể
  ...
  // === THAY ĐỔI 3: TỐI ƯU HIỂN THỊ TRÊN MÀN HÌNH RỘNG ===
  Widget _buildVnPayContent() {
    // Dùng LayoutBuilder để quyết định hiển thị 1 hay 2 cột
    return LayoutBuilder(
      builder: (context, constraints) {
        // Nếu màn hình đủ rộng, hiển thị 2 gói cạnh nhau
        ...
        // Nếu màn hình hẹp, hiển thị 2 gói theo chiều dọc
  ...
  Widget _buildIapContent() {
    ...
    // Tương tự, dùng LayoutBuilder cho IAP
    return LayoutBuilder(
      builder: (context, constraints) {
        if (constraints.maxWidth > 720) {
          return Padding(
            ...
            child: Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Expanded(child: _buildIapCard1Month()),
                const SizedBox(width: 24),
                Expanded(child: _buildIapCard12Months()),
              ],
            ),
          );
        }
  ...
  // Tách các thẻ card ra thành widget riêng để dễ quản lý và tái sử dụng
  ...
  Widget _buildVnPayCard12Months() {
    return _PackageCard(
      tier: 'ELITE', duration: '12 month', price: '\$460',
      features: const ['Receive all signals of the day', 'Analyze the reason for entering the order', 'High-precision AI signal'],
  ...
  Widget _buildIapCard1Month() {
    final product = _products['elite_1_month'];
    return _PackageCard(
      tier: 'ELITE', duration: '1 month', price: '\$78',
      features: const ['Receive all signals of the day', 'Analyze the reason for entering the order', 'High-precision AI signal'],
      onPressed: product != null ? () => _handlePurchase(product) : null,
    );
  }

  Widget _buildIapCard12Months() {
    final product = _products['elite_12_months'];
    return _PackageCard(
      tier: 'ELITE', duration: '12 month', price: '\$460',
      features: const ['Receive all signals of the day', 'Analyze the reason for entering the order', 'High-precision AI signal'],
      onPressed: product != null ? () => _handlePurchase(product) : null,
